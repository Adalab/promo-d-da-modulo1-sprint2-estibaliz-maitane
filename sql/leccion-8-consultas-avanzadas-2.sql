...
que nos ayudará a agrupar o seleccionar los resultados según los valores de algunos de los atributos de la tabla, 
y HAVING y CASE para añadir condiciones a la ejecución de ciertas partes de las consultas de manera similar a como hacemos 
con WHERE (aunque con diferencias).*/
...
SELECT COUNT(order_id), YEAR(order_date), MONTH(order_date), DAY(order_date)
	FROM orders
...
La consulta anterior nos muestra el número de pedidos para cada día concreto, pero esto es demasiado detalle.
 ...
 SELECT COUNT(order_id), YEAR(order_date), MONTH(order_date)
	FROM orders
...
SELECT  COUNT(employee_id), country, city
	FROM employees
